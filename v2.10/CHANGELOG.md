...
## v2.10 — 衣柜系统重设计 (2026-08-12)

### 重设计
- **衣柜数据全面中文化**：所有 24 件服饰名称、描述、套装名、来源全部改为简体中文
- **衣柜卡片网格**：重新设计为清爽的 4 列卡片布局，每张卡片展示图标、名称、部位标签，套装服饰右上角显示套装标签
- **服饰详情弹窗**：全新设计的详情面板，分区展示：
  - 外观信息（视觉描述）
  - 来源信息（获取方式/故事背景）
  - 套装信息（所属套装名称与描述）
  - 属性加成（好感度获取 +X%、魅力 +X% 等）
...
| 文件 | 变更 |
|------|------|
| `js/wardrobe-data.js` | 完整重写：简体中文命名 + 新增外观/来源/套装/属性四个维度 |
| `js/features.js` | 重写衣柜渲染逻辑、统一卡片生成函数、全新详情弹窗填充逻辑 |
| `css/modals.css` | 重写衣柜卡片和详情弹窗全部样式 |
| `index.html` | 重写服饰详情弹窗 HTML 结构 |
| `js/config.js` | 更新默认衣柜数据为简体中文 |

---

## v2.9 — 记忆管理 UI 优化 (2026-08-12)

### 优化
- 配置面板：从横向三列改为纵向排列
- 记忆条目：双栏分类（小总结层 / 大总结层）
- 术语统一：摘要层→小总结、元摘要层→大总结

### 新增
- 记忆导入/导出功能（JSON 格式，含配置）
...
### 新增
- 记忆管理系统：分层记忆架构（全文 → 小总结 → 大总结）
- `{{memory}}` 宏注入分层记忆上下文

### 修复
- 继续游戏：读档后直接进入游戏
- 对话记录：改用 chatHistory，上限 50 条
