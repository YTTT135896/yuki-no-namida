...
Yuki.ST_UI.openMemory = function() {
  Yuki.UI.modal.open('memory');
  Yuki.ST_UI._renderMemoryUI();
};

/**
 * Helper: render a single memory entry card.
 */
Yuki.ST_UI._renderMemEntry = function(m) {
  var date = new Date(m.timestamp).toLocaleString('zh-CN');
  var preview = m.fullText ? m.fullText.slice(0, 50) + (m.fullText.length > 50 ? '…' : '') : '（空）';
  return '<div class="mem-entry">' +
    '<div class="mem-entry-body">' +
      '<div class="mem-entry-text" title="' + m.fullText.replace(/"/g,'&quot;') + '">' + preview + '</div>' +
      '<div class="mem-entry-meta-row">' +
        '<span>' + m.speaker + '</span>' +
        '<span>' + m.chapter + '</span>' +
        '<span>' + date + '</span>' +
      '</div>' +
    '</div>' +
    '<button class="mem-entry-delete-btn" onclick="Yuki.ST_UI._deleteMemory(\'' + m.id + '\')" title="删除"><span class="material-symbols-rounded" style="font-size:.85rem">close</span></button>' +
  '</div>';
};

Yuki.ST_UI._renderMemoryUI = function() {
  var cfg = Yuki.ST.memConfig;
  document.getElementById('mem-fulltext-limit').value = cfg.fullTextLimit;
  document.getElementById('mem-summary-limit').value = cfg.summaryLimit;
  document.getElementById('mem-meta-limit').value = cfg.metaLimit;

  var total = Yuki.ST.memories.length;
  ...
  // Categorize entries (newest first within each category)
  var summaryEntries = [];  // entries in the "小总结" tier (summary tier)
  var metaEntries = [];     // entries in the "大总结" tier (meta tier)

  for (var i = total - 1; i >= 0; i--) {
    var m = Yuki.ST.memories[i];
    if (i >= fullStart) {
      // Full-text tier — not shown in either column (it's the "live" dialogue)
      continue;
    } else if (i >= sumStart) {
      summaryEntries.push(m);
    } else if (i >= metaStart) {
      metaEntries.push(m);
    }
    // beyond metaStart → hidden, not shown
  }

  document.getElementById('mem-summary-count').textContent = summaryEntries.length;
  document.getElementById('mem-meta-count').textContent = metaEntries.length;

  // Render summary column
  var sumList = document.getElementById('mem-summary-list');
  if (summaryEntries.length === 0) {
    sumList.innerHTML = '';
  } else {
    sumList.innerHTML = summaryEntries.map(function(m) { return Yuki.ST_UI._renderMemEntry(m); }).join('');
  }

  // Render meta column
  var metaList = document.getElementById('mem-meta-list');
  if (metaEntries.length === 0) {
    metaList.innerHTML = '';
  } else {
    metaList.innerHTML = metaEntries.map(function(m) { return Yuki.ST_UI._renderMemEntry(m); }).join('');
  }

  // Preview
  var preview = document.getElementById('mem-preview');
  var ctx = Yuki.ST.buildMemoryContext();
  preview.textContent = ctx || '（暂无记忆上下文。配置好层级限制后，AI 将在此视角下看到分层记忆。）';
};

Yuki.ST_UI._saveMemConfig = function() {
  var cfg = Yuki.ST.memConfig;
  cfg.fullTextLimit = Math.max(0, parseInt(document.getElementById('mem-fulltext-limit').value) || 3);
  cfg.summaryLimit = Math.max(0, parseInt(document.getElementById('mem-summary-limit').value) || 5);
  cfg.metaLimit = Math.max(0, parseInt(document.getElementById('mem-meta-limit').value) || 10);
  Yuki.ST.saveMemConfig().then(function() {
    Yuki.UI.toast('记忆配置已保存', 'success', 2000);
    Yuki.ST_UI._renderMemoryUI();
  });
};

Yuki.ST_UI._deleteMemory = function(id) {
  if (!confirm('确定删除此记忆条目？')) return;
  Yuki.ST.deleteMemory(id).then(function() {
    Yuki.ST_UI._renderMemoryUI();
    Yuki.UI.toast('记忆已删除', 'warning', 2000);
  });
};

Yuki.ST_UI._clearAllMemories = function() {
  if (!confirm('确定清空全部记忆条目？此操作不可撤销。')) return;
  Yuki.ST.clearMemories().then(function() {
    Yuki.ST_UI._renderMemoryUI();
    Yuki.UI.toast('全部记忆已清空', 'warning', 2000);
  });
};

/* ── Import / Export ── */
Yuki.ST_UI._importMemories = function() {
  var inp = document.createElement('input'); inp.type = 'file'; inp.accept = '.json';
  inp.onchange = function(e) {
    var file = e.target.files[0]; if (!file) return;
    var reader = new FileReader();
    reader.onload = function(ev) {
      try {
        ...
        if (data.memories && Array.isArray(data.memories)) {
          var added = 0;
          data.memories.forEach(function(imp) {
            var ex = Yuki.ST.memories.find(function(m) { return m.id === imp.id; });
            if (!ex) {
              Yuki.ST.memories.push(imp);
              added++;
            }
          });
          // Re-sort by timestamp
          Yuki.ST.memories.sort(function(a, b) { return a.timestamp - b.timestamp; });
          Yuki.ST.saveMemories().then(function() {
            if (data.memConfig) {
              Object.assign(Yuki.ST.memConfig, data.memConfig);
              Yuki.ST.saveMemConfig();
            }
            Yuki.ST_UI._renderMemoryUI();
            Yuki.UI.toast('已导入 ' + added + ' 条记忆' + (data.memConfig ? '（含配置）' : ''), 'success', 3000);
          });
        } else {
          Yuki.UI.toast('无效的记忆 JSON 文件', 'error', 3000);
        }
      } catch(ex) { Yuki.UI.toast('JSON 解析失败', 'error', 3000); }
    };
    reader.readAsText(file);
  };
  inp.click();
...
Yuki.ST_UI._exportMemories = function() {
  var data = {
    memories: Yuki.ST.memories,
    memConfig: Yuki.ST.memConfig,
    exportedAt: new Date().toISOString()
  };
  var blob = new Blob([JSON.stringify(data, null, 2)], {type: 'application/json'});
  var a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'yuki-memories-' + new Date().toISOString().slice(0, 10) + '.json';
  a.click();
  Yuki.UI.toast('记忆已导出（含配置）', 'success', 2000);
};

Yuki.ST_UI.initMemory = function() {
  document.getElementById('mem-save-config-btn').addEventListener('click', Yuki.ST_UI._saveMemConfig);
  document.getElementById('mem-clear-all-btn').addEventListener('click', Yuki.ST_UI._clearAllMemories);
  document.getElementById('mem-import-btn').addEventListener('click', Yuki.ST_UI._importMemories);
  document.getElementById('mem-export-btn').addEventListener('click', Yuki.ST_UI._exportMemories);
};

/* ═══════════════════════════════════════════════════
   REGEX MANAGER (NEW v2.6)
   ═══════════════════════════════════════════════════ */
Yuki.ST_UI._selRegexId = null;

Yuki.ST_UI.openRegex = function() {
  Yuki.UI.modal.open('regex');
  Yuki.ST_UI._selRegexId = Yuki.ST.regexRules.length ? Yuki.ST.regexRules[0].id : null;
  Yuki.ST_UI._renderRegexList();
  Yuki.ST_UI._renderRegexEditor();
};

Yuki.ST_UI._renderRegexList = function() {
  var list = document.getElementById('regex-list');
  list.innerHTML = Yuki.ST.regexRules.map(function(r) {
    var isSel = r.id === Yuki.ST_UI._selRegexId;
    return '<div class="lb-book-item'+(isSel?' active':'')+(r.enabled?' has-active':'')+'" data-rx-id="'+r.id+'" onclick="Yuki.ST_UI._selectRegex(\''+r.id+'\')"><span class="active-dot"></span>'+r.name+'</div>';
  }).join('');
};

Yuki.ST_UI._selectRegex = function(id) {
  Yuki.ST_UI._selRegexId = id;
  Yuki.ST_UI._renderRegexList();
  Yuki.ST_UI._renderRegexEditor();
};

Yuki.ST_UI._renderRegexEditor = function() {
  var rule = Yuki.ST.regexRules.find(function(r) { return r.id === Yuki.ST_UI._selRegexId; });
  if (!rule) {
    document.getElementById('regex-editor').style.display = 'none';
    document.getElementById('regex-empty').style.display = 'flex';
    return;
  }
  document.getElementById('regex-empty').style.display = 'none';
  document.getElementById('regex-editor').style.display = 'block';
  document.getElementById('regex-name-input').value = rule.name;
  document.getElementById('regex-pattern-input').value = rule.pattern;
  document.getElementById('regex-replacement-input').value = rule.replacement;
  document.getElementById('regex-target-select').value = rule.target;
  var toggleEl = document.getElementById('regex-toggle');
  if (rule.enabled) { toggleEl.classList.add('on'); toggleEl.setAttribute('aria-checked','true'); }
  else { toggleEl.classList.remove('on'); toggleEl.setAttribute('aria-checked','false'); }
};

Yuki.ST_UI._saveRegex = function() {
  var rule = Yuki.ST.regexRules.find(function(r) { return r.id === Yuki.ST_UI._selRegexId; });
  if (!rule) return;
  rule.name = document.getElementById('regex-name-input').value || '未命名规则';
  rule.pattern = document.getElementById('regex-pattern-input').value;
  rule.replacement = document.getElementById('regex-replacement-input').value;
  rule.target = document.getElementById('regex-target-select').value;
  rule.enabled = document.getElementById('regex-toggle').classList.contains('on');
  rule.updatedAt = Date.now();
  return Yuki.ST.saveRegexRules();
};

Yuki.ST_UI.initRegex = function() {
  // Save button
  document.getElementById('regex-save-btn').addEventListener('click', function() {
    Yuki.ST_UI._saveRegex().then(function() {
      Yuki.UI.toast('正则规则已保存', 'success', 2000);
      Yuki.ST_UI._renderRegexList();
    });
  });
  ...
  // New rule
  document.getElementById('regex-new-btn').addEventListener('click', function() {
    var name = prompt('新规则名称：', '新规则');
    if (!name) return;
    var nr = {id:'rx-'+Date.now(),name:name,pattern:'',replacement:'',target:'output',enabled:true,createdAt:Date.now(),updatedAt:Date.now()};
    Yuki.ST.regexRules.push(nr);
    Yuki.ST.saveRegexRules().then(function() {
      Yuki.ST_UI._selRegexId = nr.id;
      Yuki.ST_UI._renderRegexList();
      Yuki.ST_UI._renderRegexEditor();
      Yuki.UI.toast('规则「'+name+'」已创建', 'success', 2000);
    });
  });

  // Delete rule
  document.getElementById('regex-delete-btn').addEventListener('click', function() {
    var rule = Yuki.ST.regexRules.find(function(r) { return r.id === Yuki.ST_UI._selRegexId; });
    if (!rule) return;
    if (!confirm('确定删除规则「'+rule.name+'」吗？')) return;
    Yuki.ST.regexRules = Yuki.ST.regexRules.filter(function(r) { return r.id !== rule.id; });
    Yuki.ST.saveRegexRules().then(function() {
      Yuki.ST_UI._selRegexId = Yuki.ST.regexRules.length ? Yuki.ST.regexRules[0].id : null;
      Yuki.ST_UI._renderRegexList();
      Yuki.ST_UI._renderRegexEditor();
      Yuki.UI.toast('规则已删除', 'warning', 2000);
    });
  });

  // Test regex
  document.getElementById('regex-test-btn').addEventListener('click', function() {
    var testText = document.getElementById('regex-test-input').value;
    var pattern = document.getElementById('regex-pattern-input').value;
    var replacement = document.getElementById('regex-replacement-input').value;
    if (!pattern) { Yuki.UI.toast('请先填写正则表达式', 'warning', 2000); return; }
    if (!testText) { Yuki.UI.toast('请先输入测试文本', 'warning', 2000); return; }
    try {
      var re = new RegExp(pattern, 'g');
      var result = testText.replace(re, replacement);
      // Show result in a popup or toast
      var preview = result.length > 200 ? result.slice(0,200)+'…' : result;
      document.getElementById('regex-test-result').textContent = preview;
      document.getElementById('regex-test-result').style.display = 'block';
    } catch(e) {
      Yuki.UI.toast('正则错误：'+e.message, 'error', 4000);
      document.getElementById('regex-test-result').style.display = 'none';
    }
  });

  // Import/Export
  document.getElementById('regex-import-btn').addEventListener('click', function() {
    var inp = document.createElement('input'); inp.type='file'; inp.accept='.json';
    inp.onchange = function(e) {
      var file = e.target.files[0]; if (!file) return;
      var reader = new FileReader();
      reader.onload = function(ev) {
        try {
          ...
          if (data.regexRules && Array.isArray(data.regexRules)) {
            data.regexRules.forEach(function(r) {
              var ex = Yuki.ST.regexRules.find(function(x) { return x.id === r.id; });
              if (ex) Object.assign(ex, r);
              else Yuki.ST.regexRules.push(r);
            });
            Yuki.ST.saveRegexRules().then(function() {
              Yuki.ST_UI._renderRegexList();
              Yuki.ST_UI._renderRegexEditor();
              Yuki.UI.toast('已导入'+data.regexRules.length+'条规则', 'success', 3000);
            });
          }
        } catch(ex) { Yuki.UI.toast('无效的 JSON', 'error', 3000); }
      };
  ...
  document.getElementById('regex-export-btn').addEventListener('click', function() {
    var data = {regexRules:Yuki.ST.regexRules,exportedAt:new Date().toISOString()};
    var blob = new Blob([JSON.stringify(data,null,2)],{type:'application/json'});
    var a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'yuki-regex.json'; a.click();
    Yuki.UI.toast('正则规则已导出', 'success', 2000);
  });
};

/* ── Nav wiring (v2.6: preset → regex → world book) ── */
Yuki.ST_UI.initNav = function() {
  ...
  addItem('nav-regex', 'regular_expression', '正则管理', Yuki.ST_UI.openRegex);
  addItem('nav-lorebook', 'menu_book', '世界书', Yuki.ST_UI.openLorebook);
  addItem('nav-variables', 'database', '游戏变量', Yuki.ST_UI.openVariables);
  addItem('nav-memory', 'psychology', '记忆管理', Yuki.ST_UI.openMemory);
  addDivider();
};
