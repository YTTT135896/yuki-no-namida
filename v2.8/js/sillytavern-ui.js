...
Yuki.ST_UI._renderMemoryUI = function() {
  ...
  if (total === 0) {
    list.innerHTML = '<div style="text-align:center;padding:24px;color:var(--text-dim)">暂无记忆条目。当 AI 回复包含 &lt;summary&gt; 和 &lt;meta&gt; 标签时，自动生成记忆。</div>';
  } else {
    var entries = Yuki.ST.memories.slice().reverse();
    list.innerHTML = entries.map(function(m, i) {
      var realIdx = total - 1 - i;
      var tier, tierLabel;
      if (realIdx >= fullStart) { tier = 'full'; tierLabel = '全文'; }
      else if (realIdx >= sumStart) { tier = 'summary'; tierLabel = '摘要'; }
      else if (realIdx >= metaStart) { tier = 'meta'; tierLabel = '元摘要'; }
      else { tier = 'hidden'; tierLabel = '隐藏'; }

      var date = new Date(m.timestamp).toLocaleString('zh-CN');
      var preview = m.fullText ? m.fullText.slice(0, 60) + (m.fullText.length > 60 ? '…' : '') : '（空）';
      return '<div class="mem-entry tier-' + tier + '">' +
        '<span class="mem-tier-badge ' + tier + '">' + tierLabel + '</span>' +
        '<div class="mem-entry-body">' +
          '<div class="mem-entry-text">' + preview + '</div>' +
          ...
          '</div>' +
        '</div>' +
        '<button class="mem-entry-delete-btn" onclick="Yuki.ST_UI._deleteMemory(\'' + m.id + '\')" title="删除"><span class="material-symbols-rounded" style="font-size:.9rem">close</span></button>' +
      '</div>';
    }).join('');
  }
