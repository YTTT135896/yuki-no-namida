...
### 优化
- **配置面板**：从横向三列改为纵向排列，彻底解决溢出问题
- **记忆条目**：分栏显示——左侧「小总结层」、右侧「大总结层」，一目了然
- **术语统一**：「摘要层」→「小总结」、「元摘要层」→「大总结」
- **AI 上下文标签**：同步更新上下文注入时的层级标签

### 新增
- **记忆导入/导出**：支持 JSON 格式导入导出（含记忆配置），文件命名 `yuki-memories-YYYY-MM-DD.json`

### 修改文件
| 文件 | 变更 |
|------|------|
| `index.html` | 记忆弹窗重构：纵向配置面板、双栏条目显示、导入导出按钮 |
| `css/features.css` | 重写记忆管理样式（纵向行布局、双栏网格、精简条目卡片） |
| `js/sillytavern-ui.js` | 重写记忆渲染逻辑（分类渲染、导入导出功能） |
| `js/sillytavern-core.js` | 上下文标签更新（小总结/大总结） |
| `js/config.js` | 格式模板标签描述更新 |
