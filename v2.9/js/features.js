...
Yuki.Features.wardrobe = {
  ...
  /**
   * Render wardrobe grid from Yuki.ST.variables (v2.5 reactive).
  ...
  renderFromVars: function(charName) {
    ...
    // Build a lookup from static data: slot name → full item info
    var staticLookup = {};
    fullData.forEach(function(item) { staticLookup[item.slot] = item; });
    grid.innerHTML = slotOrder.map(function(slot, i) {
      var itemName = wardrobeVar[slot] || '—';
      var isEmpty = itemName === '—';
      var staticItem = staticLookup[slot];
      var icon = staticItem ? staticItem.icon : self.getIcon(slot);
      var set = staticItem ? staticItem.set : '';
      var hasSet = !!set;
      return '<div class="wardrobe-slot' + (isEmpty ? ' empty' : '') + '" data-char="' + charName + '" data-slot="' + slot + '"' +
        (isEmpty ? '' : ' onclick="Yuki.Features.wardrobe.openDetailBySlot(\'' + charName + '\',\'' + slot + '\')') + '>' +
        (hasSet ? '<div class="wardrobe-slot-set-badge"></div>' : '') +
        '<span class="material-symbols-rounded wardrobe-slot-icon">' + (staticItem ? staticItem.icon : 'checkroom') + '</span>' +
        '<span class="wardrobe-slot-name">' + itemName + '</span>' +
        '<span class="wardrobe-slot-label">' + slot + '</span></div>';
    }).join('');
    // Bonus: count set pieces currently worn
    var setCounts = {};
    ...
    if (bonusEl) {
      bonusEl.innerHTML = Object.entries(setCounts).map(function(e) {
        ...
        var bonus = count >= 4 ? '套装效果：全属性 +15%，魅力 +20%' : count >= 2 ? '套装效果：好感度获取 +5%' : '收集更多部件以解锁加成';
        var setDesc = '';
        fullData.forEach(function(item) { if (item.set === setName && item.setDesc) setDesc = item.setDesc; });
        return '<div style="margin-bottom:8px"><div class="wardrobe-set-bonus-label"><span class="material-symbols-rounded" style="font-size:1rem">auto_awesome</span>' + setName + '</div><div class="wardrobe-set-bonus-text">' + setDesc + '</div><div class="wardrobe-set-count">已装备 ' + count + ' 件 · ' + bonus + '</div></div>';
      }).join('') || '<div class="wardrobe-set-bonus-text" style="color:var(--text-dim)">暂无套装效果</div>';
    }
  },

  /** Open clothing detail from variable-driven wardrobe (by slot instead of index) */
  openDetailBySlot: function(charName, slot) {
    var fullData = this.getData()[charName] || [];
    var staticItem = fullData.find(function(item) { return item.slot === slot; });
    if (!staticItem) return;
    document.getElementById('clothing-detail-icon-popup').innerHTML = '<span class="material-symbols-rounded" style="font-size:2.8rem">' + staticItem.icon + '</span>';
    document.getElementById('clothing-detail-name-popup').textContent = staticItem.name;
    document.getElementById('clothing-detail-set-popup').textContent = staticItem.set ? '套装：' + staticItem.set : '散件';
    document.getElementById('clothing-detail-set-desc-popup').textContent = staticItem.setDesc || '该服饰为独立散件。';
    document.getElementById('clothing-detail-origin-popup').textContent = '来历：' + staticItem.origin;
